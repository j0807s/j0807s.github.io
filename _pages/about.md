...
* **Heterogenous Cores**: Enabled software support for arbitrary integer formats on GPUs by software-assisted packing and load balancing between Tensor, INT and FP CUDA cores [ICPP'24]. Enabled hardware support for bitslice representation to process arbitrary low-precision numeric formats [P4]. Accelerated Yinyang K-Means via dynamic thread allocation for CPU and GPU on mobile platform [P6].
* **High Performance AI Frameworks**: Developed dynamic thread allocation for a multi-tenant GPU [ESL'24], memory-efficient continual learning [P2, P7].
