...
* **Heterogenous Memory**: Developed a memory oversubscription-aware tensor migration scheduling for a GPU unified storage architecture [P1]. Enabled unified Flash Translation Layer (FTL) with dedicated IOMMUs and the optimized migration scheduler for hetergeneous memory-based DNN accelerator systems [P3]. Analyzed the behavior of a real CXL-based system on datacenter workloads to uncover open challenges in the emerging hardware platform [P5].
* **Heterogenous Processors**: Enabled software support for arbitrary integer formats on GPUs by software-assisted packing and load balancing between Tensor, INT and FP CUDA cores [ICPP'24]. Accelerated Yinyang K-Means via dynamic thread allocation for CPU and GPU on mobile platform [P6].
* **High Performance AI Frameworks**: Developed dynamic thread allocation for a multi-tenant GPU [ESL'24].  Enabled hardware support for bitslice representation to process arbitrary low-precision numeric formats on GPUs [P4]. Developed memory-efficient continual learning [P2, P7].
